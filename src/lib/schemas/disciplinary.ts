...
export const DisciplinaryRecordSchema = z.object({
  ...
  drugType: z.string().optional().default(''),
  otherIssue: z.string().optional().default(''),
  ...
  actionComments: z.string().min(10, { message: "Action comments must be at least 10 characters long." }),
    ...
    if (data.issues.includes('Drug') && !data.drugType?.trim()) {
      ctx.addIssue({
    ...
    if (data.issues.includes('Other') && !data.otherIssue?.trim()) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
